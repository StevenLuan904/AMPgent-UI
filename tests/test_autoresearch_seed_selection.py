...
from pepagent.autoresearch_seed_selection import select_ood_qualified_seed_rows
...
def test_selection_exhausts_consensus_families_before_supplemental() -> None:
    ...
    result = select_ood_qualified_seed_rows(rows, target_key="PBP2A", count=2)
...
def test_selection_is_family_unique_deterministic_and_uses_pareto_front() -> None:
    ...
    forward = select_ood_qualified_seed_rows(
        [weaker, other, better], target_key="pbp2a", count=2
    )
    reverse = select_ood_qualified_seed_rows(
...
def test_selection_excludes_structure_history_short_and_ood_rows() -> None:
    ...
    result = select_ood_qualified_seed_rows(
    ...
    assert [row["family_key_80_80"] for row in result.selected_rows] == ["f-eligible"]
...
def test_selection_fails_closed_when_unique_family_quota_is_unavailable() -> None:
    ...
    try:
        select_ood_qualified_seed_rows(rows, target_key="pbp2a", count=2)
