...
def test_compact_sync_allowlist_excludes_structures_and_trajectories():
    ...
    assert allowed_relative_path(
        PurePosixPath("acea/candidate/analysis/interface/postgresql_ingest_receipt.json")
    )
    assert allowed_relative_path(
        PurePosixPath("acea/candidate/analysis/mmgbsa/postgresql_ingest_receipt.json")
    )
