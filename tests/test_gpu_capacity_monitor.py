...
SYNTH_PROBE_TUNNEL = ROOT / "deploy" / "tunnels" / "start_synth_ampgent_probe_tunnel.ps1"
...
def test_gpu_capacity_monitor_has_scoped_host32_probe_and_wake_contract() -> None:
    ...
    assert 'SynthSshTarget = "synth@127.0.0.1"' in source
    assert "SynthSshPort = 32224" in source
    assert "StrictHostKeyChecking=accept-new" in source
    assert "UseSynthCredential" in source
    assert "synth-target.dpapi" in source
    ...
    assert 'host = "192.168.99.2"; gpu_indices = @(0, 1, 2, 3, 4, 5, 6, 7)' in source
...
def test_gpu_probe_tunnels_are_supervised_and_use_nonreserved_local_ports() -> None:
    expected = (
        (HOST19_PROBE_TUNNEL, "32222"),
        (HOST32_PROBE_TUNNEL, "32223"),
        (SYNTH_PROBE_TUNNEL, "32224"),
    )
    ...
    synth = SYNTH_PROBE_TUNNEL.read_text(encoding="utf-8")
    assert "eh002@58.34.98.79:49200" in synth
    assert "synth@192.168.99.2" in synth
    assert "synth-target.dpapi" in synth
