...
4. 工程形式从简：worker 通常只需保存 host、GPU/CPU、PID/role、source revision 和无外来冲突证明。
   ...
   不得成为独立阻塞。
5. SHA 复核从简：不得把重复核对 receipt SHA，或把 Python 可执行文件、`site-packages`、环境 manifest、
   requirements lock 组合成新的身份门，当作研究进展。内容哈希可由对象存储/幂等层自动维护，但主线验收
   是每个真实运行和调用的开始、成功/失败、重试、输入输出、目标/批次及执行上下文都已进入 PostgreSQL；
   只有文件收据而没有数据库 call/lifecycle 记录不算完成。
6. 行动默认继续：routine 缺陷先只读定位，再直接修复、做相称测试、记录并推进下一轮生成/评价；在
...
## 2026-08-24 可解释的全指标分布汇报目标

短肽探索不仅要留下行级证据，还要让实验和管理人员快速判断“整体是否变好、风险集中在哪里、极端候选
是谁”。从下一次 review 起，每个冻结打分器都必须在明确 cohort 上形成可复算的分布摘要：数值指标包含
有效/缺失/OOD、最好/最差候选、min/max、mean、median、standard deviation 与 P10/P25/P75/P90；标签
指标包含全部类别的 count/percentage 与有利方向。全体有效唯一序列、mature core/promising pool、结构池
和最终组合必须分开，且报告相对上一轮的分布位移与尾部变化。

表达方式固定为面试式通俗说明：先讲结论，首次出现的术语立即解释，最后讲局限和下一决策。分布统计是
诊断探索覆盖、模型冲突和候选质量的工具，不得从当前批次均值/分位数反推新的硬门，也不得把预测极值
称为实验冠军。每次 review 的机器可读摘要应可由 PostgreSQL Evaluation 重新生成，并保留 metric version、
单位、方向、cohort 定义和生成时间；CSV/JSON 只是导出，不替代数据库证据。

## 2026-08-24 最新总目标：六个独立靶点各150条，加1000条通用AMP

本节记录用户最新且优先级最高的项目定义，覆盖此前把“双靶点结构复核”或同一候选跨靶点结合视为终点的
规划。旧run、旧结构结果和旧决策继续作为历史计算证据保留，但不再代表当前项目是否完成。
...
1. 为六个目标分别交付150条高质量、有效、互不重复的短肽，共900条。每条短肽只需服务其所属目标
   分支，不要求同时结合其他目标。
2. 额外交付1000条不以这六个目标为条件的通用抗菌肽，单独成池。
3. 最终交付基线为1900条。生成预算必须高于1900，并依据每个分支的有效率、去重率、家族覆盖和优质
   产出率动态补充，直到每个150条分支和1000条通用池都交够，不用重复家族强行填数。
4. 每条最终候选输出序列、所属靶点/通用池、生成来源、谱系、全部适用的抗菌性/毒性/溶血/理化/
   不稳定性分数、靶序列结合预测分数、模型版本、OOD/缺失状态和可直接交付的CSV。

### 计算实验路径

- 六个靶点分支的最低靶向证据是“目标氨基酸序列 + 候选短肽序列”的序列交互预测。公开资料较少或
  缺少可靠三维结构时，直接走序列路线，结构评分可省略，不影响该分支达到150条。
- 2026-08-24公开检索的候选模型路线：PepInter作为最新纯序列交互分类主候选；PepMLM负责按目标序列
  生成并提供条件似然；CAMP作为独立序列交互复核；PepCCD作为提高条件生成多样性的challenger。
  VITAL包含几何编码，只在有可靠结构时作可选增强。正式采用前依据公开held-out/external benchmark、
  可运行权重/代码、许可和本地目标长度smoke做轻量选择。
- “高质量”由适用于该分支的完整序列评分、靶序列交互预测、家族多样性和相对排序共同定义。模型分数
  是本计算实验的结果；项目当前没有湿实验，不把实验验证、CRO采购或donor/protease合同加入完成条件。
- 结构计算只解决确实需要三维信息的候选排序问题。它是可选资源，不再构成所有目标、所有候选的固定
  门槛，也不再要求native/wrong-pocket矩阵或一条短肽横跨多个目标。

### 持续探索与汇报

后续每轮优先展示六个 `已合格/150` 和通用池 `已合格/1000`，并报告每个分支新增raw、有效唯一序列、
新家族、靶序列预测分布、12项序列指标分布和淘汰原因。一个分支交够后可继续产生challenger，但不得
挤占未交够分支。Agent收到新任务后立即进入下一轮生成、评分或策略改进，不等待定时唤醒。

表达风格以行动和结果为中心：先说本轮生成了什么、哪个分支距离交付最近、哪个分支需要换生成策略，
再给必要的模型局限。减少反复用否定句定义项目，也不把抽象“科学边界”讨论当作计算实验产出。

## 2026-08-24 长期执行goal与旧pool/winner稳定性首轮checkpoint

已建立持续执行goal：以历史6182条候选和74184项评价为基线，先完成旧pool/winner稳定性与家族覆盖审计，
再运行六个 `150` 和一个 `1000` 的生成、序列交互评分、score-all、补量与导出循环，直至1900条交付闭合。

首轮winner稳定性诊断在1139条同时通过ToxinPred3与MACREL溶血标签的候选中进行。七个方向明确且量纲
独立转换为pool内百分位的指标，经过2000组非负随机权重重排：历史39条mature core中只有1条在至少80%
的重排中仍进入前39，17条进入前39的概率低于20%；逐次删除一个指标时，只有5条在七种删项结果中
始终保留前39。当前最稳定候选是 `GRWRQWKWWWKELHHVLLDDDELL`：等权排名第1，随机权重前39概率
0.814、成为第1概率0.172、七种删项前39比例1.0。该分析说明旧Pareto核心保留了多种极端权衡，但多数
不是稳定winner；新目标可把它们作为生成种子和候选来源，必须经分支靶序列评分与稳定性复核后再交付。

生成覆盖审计同时确认：7200个raw occurrence中6182个进入有效候选、1018个invalid；共有7194个不同的
raw序列，仅6个额外occurrence来自完全相同序列。当前6182条Candidate的 `sequence_family_key` 覆盖为0，
因此下一关键动作不是继续按行数扩张，而是用完整序列identity/coverage规则补家族聚类，计算各轮新家族
增量、核心候选家族集中度和七分支可继承额度，再定向生成缺失家族。
...
`6×600 + 3000 = 6600`，不足时逐分支动态补量，不把6600当搜索上限。每个靶点首批可直接拆成
6个100条generator cell并均衡覆盖三个生成器。固定controller SHA为
...
## 2026-08-24 旧winner分数扰动稳定性增量

在既有随机权重与逐模型删项诊断之外，现增加固定等权排序下的分数扰动实验：七个方向性指标先转换为
1139条安全候选池内的百分位效用，再对每个指标加入标准差0.02的独立噪声，固定seed重复2000次。该实验
模拟软打分器的小幅数值误差，不改变原始Evaluation，也不产生新的准入硬门。

历史winner `GRWRQWKWWWKELHHVLLDDDELL` 在该扰动下进入前39的概率为1.0、成为第1的概率为0.9315；
而在随机改变目标权重时成为第1的概率仍为0.172。含义是它对小幅评分误差稳健，但对不同目标偏好的权衡
敏感。历史39条mature core中仅14条在分数扰动下以至少80%概率保留前39，22条的保留概率低于20%，
...
## 2026-08-28 结构漏斗执行覆盖：先 dG，MD 只留给极少黄金候选

长期目标继续以扩展新家族、改善活性—安全多前沿和完成12项序列证据为主体；Rosetta dG 是这些迭代
成熟之后的后置质量复核，不替代扩库。用户已授权 synth `.2 GPU2`：GPU2 用于为缺少可靠复合物的优秀
候选生成 PBP2a–peptide 坐标，Rosetta dG 使用受限 CPU 对每条200个 decoy做可复算汇总。首个 exact-once
lane 为 `pbp2a-gpu2-rosetta-20260828-v1`，同时覆盖一个已有 PepGLAD 复合物的候选和一个因子化前沿候选。
