...
- PostgreSQL 无上限严格 Pool A（2026-09-02 23:09）：300 个独立家族；AceA 73、GyrA 37、PBP2a 40、VEGFA 71、FGF2 37、ANGPT1 42；距 50 缺口分别为 0/13/10/0/13/8，其余 reserve 正在补齐 5-decoy Rosetta 与收据入库。
- synth 仅流式回传 completion receipt 与分数 JSON 到 `.19` 做身份、哈希、聚合、冲突检查及 exact-once 入库；结构不传输。
- 本地悬浮进度每 10 秒读取两端全部登记批次的 `coarse5_progress.json`，动态推导总量；状态为 `var/state/ampgent-rosetta-progress-float.json`。
