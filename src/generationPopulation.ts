...
export function formatRunGenerationPopulation(population: GenerationPopulation) {
  if (population.baseline_candidate_count === 0 && population.descendant_candidate_count === 0) {
    return '尚无可展示候选'
  }
  if (population.descendant_candidate_count === 0) {
    return `${population.baseline_candidate_count.toLocaleString()} 条基线候选`
  }
  return `${population.descendant_candidate_count.toLocaleString()} 条新生子代 · 第 ${population.max_generation} 代`
}
