...
TOP_UP_WORKFLOW_TYPE = "SevenBranchPeptideTopUpWorkflow"
...
def build_seven_branch_top_up_reservation_specs(
    schedule: SevenBranchTopUpSchedule,
) -> tuple[dict[str, Any], tuple[dict[str, Any], ...]]:
    schedule_sha256 = schedule.sha256()
    controller_identity = {
        "schema_version": "ampgent.seven-branch-top-up-controller-reservation.1",
        "run_kind": "seven_branch_peptide_design_top_up_control",
        "controller_run_id": str(schedule.controller_run_id),
        "parent_controller_run_id": str(schedule.parent_controller_run_id),
        "epoch_ordinal": schedule.epoch_ordinal,
        "schedule_sha256": schedule_sha256,
        "design_contract_sha256": schedule.design_contract.sha256(),
        "branch_keys": [item.branch_key for item in schedule.branches],
        "historical_outputs_reused": False,
    }
    controller_key = sha256_json(controller_identity)
    controller_spec = {
        **controller_identity,
        "formal_submission_key": controller_key,
        "temporal_workflow_id": f"pepagent-seven-branch-top-up-{controller_key}",
    }
    branch_by_key = {
        item.branch_key: item for item in schedule.design_contract.branches
    }
    ...
    for epoch_branch in schedule.branches:
        frozen = epoch_branch.frozen_round
        binding = frozen.request["seven_branch_round"]
        identity = {
            "schema_version": "ampgent.seven-branch-top-up-round-reservation.1",
            "run_kind": "seven_branch_design_round",
            "controller_run_id": str(schedule.controller_run_id),
            "parent_controller_run_id": str(schedule.parent_controller_run_id),
            "run_id": str(frozen.run_id),
            "branch_key": epoch_branch.branch_key,
            "branch_kind": binding["branch_kind"],
            "round_ordinal": binding["round_ordinal"],
            "epoch_ordinal": schedule.epoch_ordinal,
            "schedule_sha256": schedule_sha256,
            "design_contract_sha256": binding["design_contract_sha256"],
            "execution_contract_sha256": binding["execution_contract_sha256"],
            "expected_raw_occurrences": binding["expected_raw_occurrences"],
            "requested_delivery_count": branch_by_key[
                epoch_branch.branch_key
            ].requested_delivery_count,
            "prior_source_run_ids": [
                str(item) for item in epoch_branch.prior_source_run_ids
            ],
            "prior_evidence_snapshot_sha256": (
                epoch_branch.prior_evidence_snapshot_sha256
            ),
            "top_up_plan_sha256": epoch_branch.top_up_plan.sha256(),
            "historical_outputs_reused": False,
        }
        child_specs.append(
            {
...
async def reserve_seven_branch_top_up_schedule(
    *, schedule: SevenBranchTopUpSchedule, target_manifest: dict[str, Any]
) -> dict[str, Any]:
    ...
    all_ids = [
        schedule.controller_run_id,
        *(item.frozen_round.run_id for item in schedule.branches),
    ]
    lock_id = int.from_bytes(
        bytes.fromhex(schedule.sha256())[:8], byteorder="big", signed=True
    )
    async with SessionFactory() as session, session.begin():
        ...
        parent = await session.get(ExperimentRun, schedule.parent_controller_run_id)
        if parent is None or parent.status != "succeeded":
            raise ValueError("top-up parent controller is not durably succeeded")
        existing = list(
        ...
        if existing:
            if len(existing) != len(all_ids):
                raise ValueError("seven-branch top-up reservation is partially present")
            by_id = {item.id: item for item in existing}
            expected = {schedule.controller_run_id: controller_spec}
            for epoch_branch, spec in zip(
                schedule.branches, child_specs, strict=True
            ):
                stored_run = by_id[epoch_branch.frozen_round.run_id]
                expected[stored_run.id] = {
                    **spec,
                    "target_id": str(stored_run.target_id),
                    "target_binding_role": (
                        "frozen_target_sequence"
            ...
            if any(item.spec_json != expected[item.id] for item in existing):
                raise ValueError("existing seven-branch top-up reservation drifted")
            return {
                "created": False,
                "controller_run_id": str(schedule.controller_run_id),
                "child_run_ids": [
                    str(item.frozen_round.run_id) for item in schedule.branches
                ],
                "schedule_sha256": schedule.sha256(),
                "schedule_artifact_sha256": stored_artifact.sha256,
            }
        ...
        session.add(
            ExperimentRun(
                ...
                status="created",
                temporal_workflow_id=controller_spec["temporal_workflow_id"],
                parent_run_id=schedule.parent_controller_run_id,
            )
        )
        for epoch_branch, spec in zip(schedule.branches, child_specs, strict=True):
            branch_key = spec["branch_key"]
            target = targets.get(branch_key, anchor)
            stored_spec = {
                **spec,
            ...
            session.add(
                ExperimentRun(
                    id=epoch_branch.frozen_round.run_id,
                    target_id=target.id,
                    spec_json=stored_spec,
                    spec_sha256=sha256_json(stored_spec),
                    formal_submission_key=spec["formal_submission_key"],
                    status="created",
                    temporal_workflow_id=epoch_branch.frozen_round.workflow_id,
                    parent_run_id=schedule.controller_run_id,
                )
            )
        await session.flush()
        await session.execute(
            ...
            .values(
                ...
                storage_uri=stored_artifact.uri,
                metadata_json={
                    "role": "seven_branch_top_up_schedule",
                    "immutable": True,
                    "controller_run_id": str(schedule.controller_run_id),
                    "parent_controller_run_id": str(
                        schedule.parent_controller_run_id
                    ),
                },
            )
            .on_conflict_do_nothing(index_elements=[Artifact.sha256])
        ...
        await ExperimentRepository(session).append_event(
            "run",
            schedule.controller_run_id,
            "seven_branch.top_up_schedule_reserved",
            "seven-branch-reservation-cli",
            {
                "schedule_sha256": schedule.sha256(),
                "schedule_artifact_sha256": stored_artifact.sha256,
...
async def submit_reserved_seven_branch_top_up_schedule(
    *, schedule: SevenBranchTopUpSchedule
) -> dict[str, Any]:
    controller_spec, _ = build_seven_branch_top_up_reservation_specs(schedule)
    settings = get_settings()
    client = await Client.connect(
        settings.temporal_address, namespace=settings.temporal_namespace
    )
    identity = {
        "workflow_type": TOP_UP_WORKFLOW_TYPE,
        "schedule_sha256": schedule.sha256(),
        "controller_run_id": str(schedule.controller_run_id),
    }
    workflow_id = controller_spec["temporal_workflow_id"]
    try:
        ...
        handle = client.get_workflow_handle(workflow_id)
        description = await handle.describe()
        if getattr(description, "workflow_type", None) != TOP_UP_WORKFLOW_TYPE:
            raise ValueError("existing top-up workflow type drifted") from error
        memo = getattr(description, "memo", None)
        if not isinstance(memo, dict) or memo.get(MEMO_KEY) != identity:
            raise ValueError("existing top-up submission identity drifted") from error
    description = await handle.describe()
    temporal_run_id = str(getattr(description, "run_id", "") or "")
    if not temporal_run_id:
        raise ValueError("seven-branch top-up submission returned no run identity")
    lock_id = int.from_bytes(
        bytes.fromhex(schedule.sha256())[:8], byteorder="big", signed=True
    )
    async with SessionFactory() as session, session.begin():
        ...
        run = await session.get(ExperimentRun, schedule.controller_run_id)
        if run is None or run.spec_json != controller_spec:
            raise ValueError("seven-branch top-up reservation is missing or drifted")
        if run.temporal_run_id not in (None, temporal_run_id):
            raise ValueError("seven-branch top-up Temporal run identity drifted")
        already_recorded = run.temporal_run_id == temporal_run_id and run.status == "running"
        run.temporal_run_id = temporal_run_id
        run.status = "running"
        if not already_recorded:
            await ExperimentRepository(session).append_event(
                "run",
                run.id,
                "seven_branch.top_up_workflow_submitted",
                "seven-branch-reservation-cli",
                {
